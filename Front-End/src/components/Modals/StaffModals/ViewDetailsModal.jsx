...
const ViewDetailsModal = ({ data, onClose, handleTriggerAPI, handleEditModal, triggerAPI, roleLoggedIn }) => {
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        ...
        if (data.role === "manager") {
          combinedData.baseSalary = staffData.baseSalary;
          combinedData.overtimeRate = staffData.overtimeRate;
        }
          ...
          <div className="modal-body px-4 py-3" style={{ maxHeight: '60vh', overflowY: 'auto' }}>
            <div className="row g-3">
              {Object.entries(userData)
                .filter(([key]) =>
                  roleLoggedIn.roleLoggedIn.toLowerCase().trim() === "manager" || (key !== "baseSalary" && key !== "overtimeRate")
                )
                .map(([key, value]) => (
